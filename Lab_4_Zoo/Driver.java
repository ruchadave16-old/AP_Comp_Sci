	...
	public static void main(String args[]) throws IOException, ClassNotFoundException {
		...
		while (!action.equals("exit")) {
			...
			if (action.equals("new")) {
				...
				zoo.setName(name);
				Zone[] allZones = zoo.getAllZones();
				zoo.zoneSort(allZones);
			}
			...
			if (action.equals("saved")) {
				...
				zoo.setName(fileName.substring(0, fileName.length() - 4));
				for (String s: allZones) {
					zoo.readZoneData(s);
				}
				Zone[] zones = zoo.getAllZones();
				zoo.zoneSort(zones);	
			}
